...
def get_argparse():
    ...
    parser.add_argument("--caption",type=str,default="a sculpture of a brown cat",help = "caption for the image")
    ...
    parser.add_argument("--no_view",default=True,action='store_false',help = "flag to mark if additional message of view is needed by caption")
...
def train_scene(args): 
    ...
    if(args.load_model is not None): net.load_state_dict(torch.load(args.load_model + ".pth"))
    for epoch in tqdm(range(args.epoch)):
        running_loss = 0.0
        for i in range(4):
            time0 = time.time()
            # phi_list = np.pi/3*np.random.random(view_num) +(i/2-1/6)*np.pi
            phi_list = np.array([np.pi*i/2])
            # print(phi_list/np.pi)
            c2w = get_c2w(theta,phi_list)    
            optimizer.zero_grad()
            clean_optimizer.zero_grad()
            time1 = time.time()
            # background = math.cos(epoch/1000*math.pi)/3+2/3
            background = 1/2+1/8*torch.rand((view_num,resolution[0],resolution[1],3),device=device)
            color_imgs,trans_imgs,weights = render_image(net,c2w,intrinsics,resolution,background,device)
            time2 = time.time()
            loss = calcu_clip_loss(color_imgs,trans_imgs,caption,clip_model,clip_processor,i,device)
            time3 = time.time()
            loss.backward()
            time4 = time.time()
            optimizer.step()
        ...
        if((epoch+1) % args.save_num == 0):torch.save(net.state_dict(),"gauss_new_net_"+str(epoch+1)+".pth")

    plt.switch_backend('Agg') 
    plt.plot(epoch_losses,'b',label = 'loss')        
    plt.ylabel('loss')
    plt.xlabel('epoch')
    plt.savefig("gauss_new_net" + ".jpg") 
